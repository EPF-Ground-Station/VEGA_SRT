...
def simulate(l, b, beamwidth=0.6, v_min=-400, v_max=400, plot_file=''):
    ...
    try:
        response = requests.get('https://www.astro.uni-bonn.de/hisurvey/euhou/LABprofile/download.php?ral=' +
                                str(l)+'&decb='+str(b)+'&csys=0&beam='+str(beamwidth))
    except requests.exceptions.ConnectionError:
        raise requests.exceptions.ConnectionError(
            'Failed to reach astro.uni-bonn.de. Make sure you are connected to the internet and try again.')
    ...
    left_frequency_edge = 1420.4057517667 + \
        1420.4057517667e6 * -v_max/(299792458 * 1e6)
    right_frequency_edge = 1420.4057517667 + \
        1420.4057517667e6 * -v_min/(299792458 * 1e6)
    ...
    try:
        plt.title('Simulated HI Profile $(l$=$'+str(l)+'\degree$, $b$=$' +
                  str(b)+'\degree)$ | Beamwidth: $'+str(beamwidth)+'\degree$', pad=40)
    except:  # Catch missing TeX exception
        plt.title('Simulated HI Profile (l='+str(l)+' deg, b='+str(b) +
                  ' deg) | Beamwidth: '+str(beamwidth)+' deg', pad=40)
    ...
    ax_secondary.axvline(x=0, color='brown', linestyle='--',
...
def predict(lat, lon, height=0, source='', date='', plot_sun=True, plot_file=''):
    ...
    if source != '':
        seconds = float(delta_midnight[np.argmax(
            obj_altaz.alt*u.deg)]*3600/u.hour)
        max = seconds/3600

        plt.axvline(x=max, color='brown', linestyle='--',
...
def map_hi(ra=None, dec=None, plot_file=''):
    ...
    if sys.version_info >= (3, 4):
        ...
        virgo_path = importlib.util.find_spec(
            'virgo').submodule_search_locations[0]
    else:
        import imp
    ...
    # Plot map
    plt.imshow(survey_corrected, extent=[
               24, 0, -90, 90], aspect=0.07, interpolation='gaussian')
    ...
    if ra is not None and dec is not None:
        if ra >= 0 and ra <= 24 and dec >= -90 and dec <= 90:
            plt.axvline(ra, 0, 1, linestyle='--', linewidth=2,
                        color=(0.85, 0.15, 0.16, 0.9))
            plt.axhline(dec, 0, 1, linestyle='--', linewidth=2,
                        color=(0.85, 0.15, 0.16, 0.9))
            plt.scatter(ra, dec, s=200, color=[0.85, 0.15, 0.16])
        else:
            warnings.warn(
                'RA and/or Dec out of range. Ensure the input is decimal hours and decimal degrees respectively.')

    if plot_file != '':
        # Add survey citation
        plt.text(5.62, 92.1, 'LAB HI Survey (Kalberla et al., 2005)',
                 fontsize=14, bbox={'facecolor': 'white', 'pad': 4})
...
def plot(obs_parameters='', n=0, m=0, f_rest=0, slope_correction=False, dB=False, vlsr=False, meta=False, avg_ylim=[0, 0], cal_ylim=[0, 0], rfi=[], xlim=[0, 0], ylim=[0, 0], dm=0,
         obs_file='observation.dat', cal_file='', waterfall_fits='', spectra_csv='', power_csv='', plot_file='plot.png', avplot_file="average.png", calplot_file="calibrated.png",
         waterplot_file="waterfall.png", powplot_file="powerDist.png"):
    '''
    ...
    '''
    import matplotlib
    # Try commenting this line if you run into display/rendering errors
    matplotlib.use('Agg')
    import matplotlib.pyplot as plt
    ...
    def SNR(spectrum, mask=np.array([])):
        ...
        noise = np.nanstd((spectrum[2:]-spectrum[:-2])
                          [mask[1:-1] == 0])/np.sqrt(2)
    ...
    def best_fit(power):
        ...
        gaussian_fit_y = 1.0/np.sqrt(2*np.pi*var) * \
            np.exp(-0.5*(gaussian_fit_x-avg)**2/var)
    ...
    if obs_parameters != '':
        ...
        warnings.warn(
            'No observation parameters passed. Attempting to load from header file ('+header_file+')...')
        ...
        for i in range(len(headers)):
            if 'mjd' in headers[i]:
                ...
                loc = tuple(
                    map(float, headers[i].strip().split('=')[1].split(' ')))
            elif 'ra_dec' in headers[i]:
                if headers[i] != 'ra_dec=':
                    ra_dec = tuple(
                        map(str, headers[i].split('=')[1].split(' ')))
                else:
                    ra_dec = None
            elif 'az_alt' in headers[i]:
                if headers[i] != 'az_alt=':
                    az_alt = tuple(
                        map(float, headers[i].split('=')[1].split(' ')))
                else:
                    az_alt = None
    ...
    if vlsr or meta:
        ...
        obs_time = obs_location.get_itrs(
            obstime=Time(str(mjd), format='mjd', scale='utc'))

        if az_alt:
            obs_coord = SkyCoord(az=az_alt[0]*u.degree, alt=az_alt[1]*u.degree, frame='altaz',
                                 location=obs_location, obstime=Time(str(mjd), format='mjd', scale='utc'))
            obs_coord = obs_coord.icrs
            print(obs_coord)
        elif ra_dec:
            obs_coord = SkyCoord(ra=ra_dec[0]*u.degree,
                                 dec=ra_dec[1]*u.degree, frame='icrs')
        else:
            obs_coord = None
    ...
    left_velocity_edge = -299792.458 * \
        (bandwidth-2*frequency+2*f_rest)/(bandwidth-2*frequency)
    right_velocity_edge = 299792.458 * \
        (-bandwidth-2*frequency+2*f_rest)/(bandwidth+2*frequency)
    ...
    offset = 1
    waterfall = offset * \
        np.fromfile(obs_file, dtype='float32').reshape(-1, channels)/bins
    ...
    if rfi != []:
        ...
        for j in range(len(rfi)):
            ...
            current_rfi = rfi[j]
            rfi_lo = channels * \
                (current_rfi[0] - (frequency - bandwidth/2))/bandwidth
            rfi_hi = channels * \
                (current_rfi[1] - (frequency - bandwidth/2))/bandwidth
    ...
    if cal_file != '':
        waterfall_cal = offset * \
            np.fromfile(cal_file, dtype='float32').reshape(-1, channels)/bins
    ...
    mask[np.logical_and(frequency > f_rest*1e-6-0.2, frequency <
                        f_rest*1e-6+0.8)] = 1  # Margins OK for galactic HI
    ...
    if waterfall_fits != '':
        ...
        try:
            ...
            warnings.warn(
                'Observation MJD could not be found and will not be part of the FITS header.')
            pass
    ...
    if spectra_csv != '':
        if cal_file != '':
            np.savetxt(spectra_csv, np.concatenate((frequency.reshape(channels, 1),
                                                    avg_spectrum.reshape(
                channels, 1), avg_spectrum_cal.reshape(channels, 1),
    ...
    if meta:
        ...
        meta_title = 'Date and Time: ' + \
            time.strftime('%Y-%m-%d %H:%M:%S %Z', time.localtime(epoch))
        if obs_coord:
            meta_title += '          Target: ' + \
                obs_coord.to_string('hmsdms', precision=0) + \
                ' in ' + get_constellation(obs_coord)
        meta_title += '\n'
    ...
    for (figure, ax) in [(figAv, axAv), (fig, ax1)]:
        ...
        ax.set_xlabel('Frequency (MHz)')
        if avg_ylim != [0, 0]:
            ax.set_ylim(avg_ylim[0], avg_ylim[1])
        if dB:
        ...
        if xlim == [0, 0] and f_rest != 0:
            ...
            ax_secondary = ax.twiny()
            ax_secondary.set_xlabel('Radial Velocity (km/s)', labelpad=5)
            ax_secondary.axvline(x=0, color='brown',
                                 linestyle='--', linewidth=2, zorder=0)
            ax_secondary.annotate('Spectral Line\nRest Frequency', xy=(460-text_offset, 5),
    ...
    if cal_file != '':
        ...
        for (figure, ax) in [(figCal, axCal), (fig, ax2)]:
            ...
            if cal_ylim != [0, 0]:
                ax.set_ylim(cal_ylim[0], cal_ylim[1])
            else:
                ax.set_ylim()
            ...
            if n != 0:
                if f_rest != 0:
                    ax.legend(bbox_to_anchor=(0.002, 0.96), loc='upper left')
                else:
                    ax.legend(loc='upper left')
    ...
    figWater, axWater = plt.subplots()

    for (figure, ax) in [(figWater, axWater), (fig, ax3)]:

        ax.imshow(decibel(waterfall), origin='lower', interpolation='None', aspect='auto',
        ...
        ax.set_title('Dynamic Spectrum (Waterfall)')

    figWater.tight_layout()
    figWater.savefig(waterplot_file)
    ...
    if m != 0:
        ax4.plot(t, power_clean, color='orangered',
                 label='Median (n = '+str(m)+')')
        ax4.set_ylim()
    if ylim == [0, 0]:
    ...
    figPow, axPow = plt.subplots()

    for (figure, ax) in [(figPow, axPow), (fig, ax5)]:

        ax.hist(power, np.max([int(np.size(power)/50), 10]), density=1,
                alpha=0.5, color='royalblue', orientation='horizontal', zorder=10)
        ax.plot(best_fit(power)[1], best_fit(power)[0], '--',
                color='blue', label='Best fit (Raw)', zorder=20)
        if m != 0:
            ax.hist(power_clean, np.max([int(np.size(power_clean)/50), 10]), density=1,
                    alpha=0.5, color='orangered', orientation='horizontal', zorder=10)
            ax.plot(best_fit(power_clean)[1], best_fit(power_clean)[
                0], '--', color='red', label='Best fit (Median)', zorder=20)
        ax.set_xlim()
        ax.set_ylim()
        ax.get_shared_x_axes().join(ax5, ax4)
        ax.set_yticklabels([])
...
def plot_rfi(rfi_parameters, data='rfi_data', dB=True, plot_file='plot.png'):
    ...
    n = len([f for f in os.listdir(data) if f.endswith(
        '.dat') and os.path.isfile(os.path.join(data, f))])

    for i in range(int(n)):
        # Load data
        waterfall = offset * \
            np.fromfile(data+'/'+str(i)+'.dat',
                        dtype='float32').reshape(-1, channels)/bins
    ...
    for i in range(int(n)):
        f_total = np.linspace((f_lo+bandwidth*i)-0.5*bandwidth,
                              (f_lo+bandwidth*i)+0.5*bandwidth, channels, endpoint=False)*1e-6
...
def monitor_rfi(f_lo, f_hi, obs_parameters, data='rfi_data'):
    ...
    for frequency in range(int(f_lo), int(f_hi), int(bandwidth)):
        ...
        # Run RFI monitor
        observe(obs_parameters=rfi_parameters, spectrometer='ftf',
                obs_file=data+'/'+str(i)+'.dat')
        i += 1
...
def main():
    ...
    observe(obs_parameters=observation,
            obs_file=args.obs_file, start_in=args.start_in)
